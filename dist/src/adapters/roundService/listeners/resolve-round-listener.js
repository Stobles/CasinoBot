...
import { getResolveRoundError, resolveRound, } from "@/features/round/resolve-round.js";
import { ROULETTE_COLORS_MAP } from "@/kernel/game/roulette/types.js";
import { bot } from "@/shared/lib/bot.js";
export function registerResolveRoundListener(eventService) {
    eventService.on("resolveRound", async ({ data }) => {
        ...
        if (gameRound.type === "Left") {
            ...
            if (error)
                bot.telegram.sendMessage(chatTelegramId.toString(), error);
            return;
        }
        if (!gameRound.value.result)
            return;
        const payout = await payoutBet(roundId, gameRound.value.result);
        if (payout.type === "Left" && payout.value === "no-bets") {
            bot.telegram.sendMessage(chatTelegramId, `🎯 Выпало: ${gameRound.value.result.number} (${ROULETTE_COLORS_MAP[gameRound.value.result.color]})\nСтавок не было, деньги остаются у меня`);
            return;
        }
        if (payout.type === "Left") {
        ...
        const winnersText = winnerUsers.length > 0
            ? winnerUsers.map((b) => `- @${b.username}`).join("\n")
            : "Отсутствуют";
        const losersText = loserUsers.length > 0
...
//# sourceMappingURL=resolve-round-listener.js.map