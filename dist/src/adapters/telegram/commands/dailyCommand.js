...
import { dailyPayout, getDailyPayoutError, } from "@/features/payout/daily-payout.js";
import { ensureSession } from "@/features/session/ensure-session.js";
export function registerDailyCommand(bot) {
    bot.command("daily", async (ctx) => {
        ...
        if (dailyPayoutResult.type === "Left") {
            ...
            if (error)
                await ctx.sendMessage(error, { parse_mode: "MarkdownV2" });
            return;
        }
        await ctx.sendMessage(`💸 Лови монету, братик (+${DAILY_PAYOUT})`);
    });
}
//# sourceMappingURL=dailyCommand.js.map